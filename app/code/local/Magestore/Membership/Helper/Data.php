...
class Magestore_Membership_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function isProductDiscount($customerId, $productId){
		...
		foreach($collection as $item){
			...
			$productIds = array_merge($package->getAllProductIds(),$productIds);
		}
	...
	*/
	public function getMemberpackage($customerId, $productId){
		$packages = array();
		$memberId = $this->getMemberId($customerId);
		$collection = Mage::getModel('membership/memberpackage')->getCollection()
						->addFieldToFilter('member_id', $memberId)
	...
	public function isSignedUpPackage($memberId,$packageId)
	{
		$memberpackage = Mage::getModel('membership/memberpackage')->getCollection()
							...
							->getFirstItem();
		if($memberpackage->getId()) return $memberpackage;
		else return null;	
	}
	...
	public function getProductDiscountPercent($productId, $package){
		...
		if(count($groupIds)){
			...
			$pos = strpos($productPrice, '%');								
			if($pos === false && $productPrice)
				return 1;
			else
				return floatval($productPrice)/100;
	...
	*/
	...
	public function addPackageToMember($memberId, $packageId, $orderId){
		try{
			...
			if(count($memberPackage_collection)) //the package is already bought
			{
				...
				//update the end time
				$endTime = date('Y-m-d H:i:s',strtotime($memberPackage->getEndTime() . '+' . $package->getDuration() . ' months' ));
			...
			{
				$endTime = date('Y-m-d H:i:s',strtotime(now() . '+' . $package->getDuration() . ' months' ));
	...
	public function addPaymentHistory($memberId,$packageId,$orderId)
	{
		...
		$startTime = now();
		$endTime = date('Y-m-d H:i:s',strtotime($startTime . '+' . $package->getDuration() . ' months' ));
		$paymentHistory = Mage::getModel('membership/paymenthistory')->getCollection()
								...
								->getFirstItem();
		if($paymentHistory->getId()) return;						
	...
	{	
		...
		$collection = Mage::getModel('membership/memberpackage')->getCollection()
						->addFieldToFilter('package_id', $packageId)
						->addFieldToFilter('member_id', array('nin'	=> $memberIds));		
		...
		if (count($memberIds)){
			foreach ($memberIds as $memberId){			
				if (!in_array($memberId, $listMemberIds)){				
					...
					$endTime = date('Y-m-d H:i:s',strtotime(now() . '+' . (int)$duration . ' months' ));
	...
	public function getJoinToPackage($id){
		if($id && isset($id)){
			...
			$collection->getSelect()->joinLeft(array('c' =>$collection->getTable('membership/memberpackage')),
							'main_table.package_id = c.package_id and c.member_id = '.$id, 
							array('end_time', 'status', 'bought_item_total', 'saved_total'));						
			return $collection;
		}
		else{
		...
		if (count($packageIds)){
			foreach ($packageIds as $packageId){						
				if (!in_array($packageId, $listPackageIds)){				
					$duration = Mage::getModel('membership/package')->load($packageId)->getDuration();
					$endTime = date('Y-m-d H:i:s',strtotime(now() . '+' . $duration . ' months' ));
					$model = Mage::getModel('membership/memberpackage');
					$memberpackage = array(
						'package_id'		=> $packageId,
						'member_id'			=> $memberId,
